# Starting Point for DGX Upgrade, 4.6.0 : 
## Step 1: Starting Point for Upgrade to Latest 4.XXX: 
...
This will maintain that the same pvc claims are used throughout the install.
...
### 1.1 Install the nvidia-release-upgrade package for upgrading to the latest DGX OS 4 release.
```
sudo apt install -y nvidia-release-upgrade
```
### 1.2 Issue the following only if upgrading from DGX OS 4.99.
```
sudo apt install -y nvidia-fabricmanager-450/bionic-updates
--allow-downgrades
```
#### 1.3 Start the DGX OS release upgrade process.
```
...
#### 1.5 Wait for the upgrade process to complete, and press return or n at the prompt that appears when the system upgrade is completed.
##### 1.7 Check if nvidia-fabricmanager-<version>, libnvidia-nscq-<version>, or both packages need to be updated by running the following command.
```
dpkg -l | grep -E -i  'nscq|fabricmanager'
```
##### 1.8 If nvidia-fabricmanager-<version>, libnvidia-nscq-<version>, or both packages are installed, compare the installed versions with the NVIDIA Server Driver. 
```
dpkg -l | grep 'NVIDIA Server Driver metapackage'
```
##### 1.9 If the NVIDIA Server Driver version is newer than any of the two packages, the packages must be updated to the same major version as the NVIDIA Server Driver. For example,
```
...
##### 1.20 Restart the system to complete the update process.
```
sudo reboot
```
...
#### 4.1 For the minimum Linux kernel version of the release to which you are upgrading, refer to the release notes for that release.
#### 4.2 Confirm the NVIDIA Graphics Drivers for Linux version.
```
nvidia-smi
```

### 3. Recovering from an Interrupted or Failed Update
...
#### 5.1 Reconfigure the packages.

```
dpkg -a -configure
```
#### 5.2 Fix the broken package installs.

```
apt -f install -y
```
#### 5.3 Determine where the release-upgrader was extracted.
```
/tmp/ubuntu-release-upgrader-<random-string>
```
#### 5.4 Determine where the release-upgrader was extracted.
```
sudo bash
...
```
#### 5.5 Do Not Reboot
#### 5.6 Issue the following command and reboot

```
bash /usr/bin/nvidia-post-release-upgrade
reboot
```
## Step 4: Performing the Release Upgrade to 6.XX
### 1.0 Preparing to Upgrade the Software.
### 1.1 Connect to the DGX System Console.
...
### 1.2 Verifying the DGX System Connection to the Repositories.
...
### 2.0 Upgrade DGX OS 5 to the Latest Version.
#### 2.1 Download information from all configured sources about the latest versions of the packages.
...
#### 2.2 Download information from all configured sources about the latest versions of the packages.
```
sudo apt -y full-upgrade
```
...
### 3.0 Performing the Release Upgrade.
#### 3.1 Install the nvidia-release-upgrade package for upgrading to the latest DGX OS 5 release
```
sudo apt install -y nvidia-release-upgrade
```
#### 3.2 Start the DGX OS release upgrade process.
```
sudo nvidia-release-upgrade
```
...
#### 3.4 Wait for the upgrade process to complete and press y at the prompt that appears when the system upgrade is completed.
```
